    ...
    <header className="sticky top-0 z-40 w-full bg-white/80 backdrop-blur-md border-b border-slate-100 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <nav className="hidden md:flex items-center gap-8 text-xs font-semibold text-slate-600">
            <button onClick={() => handleScrollTo('features')} className="hover:text-slate-900 transition-colors cursor-pointer">Features</button>
            <button onClick={() => handleScrollTo('benefits')} className="hover:text-slate-900 transition-colors cursor-pointer">Benefits</button>
            <button onClick={() => handleScrollTo('how-it-works')} className="hover:text-slate-900 transition-colors cursor-pointer">How It Works</button>
            <button onClick={() => handleScrollTo('pricing')} className="hover:text-slate-900 transition-colors cursor-pointer">Pricing</button>
            <button onClick={() => handleScrollTo('faq')} className="hover:text-slate-900 transition-colors cursor-pointer">FAQ</button>
          </nav>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-b border-slate-100 px-4 pt-2 pb-6 space-y-3 shadow-lg flex flex-col text-sm font-semibold text-slate-600">
          <button onClick={() => handleScrollTo('features')} className="py-2 text-left hover:text-slate-950 border-b border-slate-50 cursor-pointer">Features</button>
          <button onClick={() => handleScrollTo('benefits')} className="py-2 text-left hover:text-slate-950 border-b border-slate-50 cursor-pointer">Benefits</button>
          <button onClick={() => handleScrollTo('how-it-works')} className="py-2 text-left hover:text-slate-950 border-b border-slate-50 cursor-pointer">How It Works</button>
          <button onClick={() => handleScrollTo('pricing')} className="py-2 text-left hover:text-slate-950 border-b border-slate-50 cursor-pointer">Pricing</button>
          <button onClick={() => handleScrollTo('faq')} className="py-2 text-left hover:text-slate-950 border-b border-slate-50 cursor-pointer">FAQ</button>
          
          <div className="pt-4 flex flex-col gap-2.5">
            <button 
              onClick={() => { setMobileMenuOpen(false); onSignIn?.(); }}
              className="w-full text-center bg-white border border-slate-200 hover:bg-slate-50 text-slate-800 font-bold py-2.5 rounded-xl cursor-pointer"
            >
              Sign in
            </button>
            <button 
