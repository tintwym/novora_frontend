...
export default function AddEmployeeModal({ isOpen, onClose, onAddEmployee, addToast }: AddEmployeeModalProps) {
  ...
  // Sync / Reset wizard when open (defer setState to avoid cascading render in effect)
  useEffect(() => {
    if (!isOpen) return
      ...
      <header id="creation-header-ribbon" className="bg-white border-b border-slate-200/70 h-16 px-8 flex items-center justify-between sticky top-0 z-10 shrink-0">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            onClick={() => {
              addToast('Progress saved to offline workspace draft.', 'success');
              onClose();
            }}
            className="px-4.5 py-2 text-xs font-black text-slate-700 bg-white border border-slate-200/80 hover:bg-slate-50 rounded-xl transition-all shadow-2xs"
          >
            Save as draft
          </button>
          ...
          <button 
            ...
            onClick={onClose}
            className="px-4.5 py-2 text-xs font-black text-slate-500 bg-white border border-slate-200 hover:bg-slate-50 hover:text-slate-800 rounded-xl transition-all shadow-2xs"
          >
            Cancel
          </button>
          ...
          <button 
            ...
            onClick={() => addToast('Wizard templates are currently locked.', 'info')}
            className="h-9 w-9 bg-white hover:bg-slate-55 border border-slate-200 text-slate-500 rounded-xl flex items-center justify-center cursor-pointer transition-colors"
          >
          ...
          {[1, 2, 3, 4, 5].map((sIndex) => {
            ...
            return (
              <React.Fragment key={sIndex}>
                ...
                >
                  <div className={`h-8 w-8 rounded-full flex items-center justify-center font-bold text-xs transition-all ${
                    isCompleted 
                      ...
                      : isActive 
                        ? 'border-2 border-novora text-novora bg-blue-50/50' 
                        : 'bg-slate-100 text-slate-400 group-hover:bg-slate-200'
                  }`}>
                    {isCompleted ? <Check className="h-4.5 w-4.5" strokeWidth={3} /> : sIndex}
                  </div>
                  <span className={`text-xs transition-colors ${
                    isActive ? 'text-novora font-black' : isCompleted ? 'text-slate-700 font-bold' : 'text-slate-400 font-bold group-hover:text-slate-600'
                  }`}>
                    {labelText}
                  </span>
                </div>
                ...
                {sIndex < 5 && (
                  <div className={`flex-1 h-0.5 mx-4 transition-all ${step > sIndex ? 'bg-novora' : 'bg-slate-100'}`} />
                )}
              </React.Fragment>
            );
          })}
            ...
            <div id="form-step-1" className="space-y-6 animate-in fade-in duration-300">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs font-semibold text-slate-600 w-full">
                    <label className="flex items-center gap-3 p-3 bg-slate-50/50 hover:bg-slate-50 rounded-2xl border border-slate-100 cursor-pointer select-none transition-colors">
                      <input 
                        ...
                        className="h-4 w-4 text-novora rounded border-slate-300 focus:ring-blue-500" 
                      />
              ...
              <div className="bg-white border border-slate-200/60 rounded-3xl p-6.5 shadow-xs space-y-6">
                ...
                <div className="space-y-4">
                  <div className="flex text-[10px] font-black text-novora uppercase tracking-widest bg-blue-50/70 py-0.5 px-2.5 rounded w-fit">
                    Employment
                  </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex gap-2">
                        <input 
                          ...
                          className="flex-1 bg-white border border-slate-200 focus:outline-none focus:border-novora p-2.5 rounded-xl font-bold text-slate-800"
                        />
                        <button 
                          type="button"
                  ...
                  <div className="space-y-4 text-xs font-semibold">
                    <div className="space-y-1.5">
                      ...
                      <textarea 
                        ...
                        rows={3} 
                        className="w-full bg-white border border-slate-200 focus:outline-none focus:border-novora p-3 rounded-2xl text-slate-800 placeholder-slate-400 font-bold"
                      />
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input 
                        ...
                        className="w-full bg-white border border-slate-200 focus:outline-none focus:border-novora p-2.5 rounded-xl font-bold text-slate-800 font-mono tracking-tight"
                      />
                    </div>
                ...
                <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input 
                      ...
                      className="h-4 w-4 text-novora rounded border-slate-300 focus:ring-blue-500 cursor-pointer" 
                    />
                ...
                <div className={`grid grid-cols-1 md:grid-cols-2 gap-5 text-xs transition-opacity duration-250 ${passportEnabled ? 'opacity-100' : 'opacity-40 cursor-not-allowed pointer-events-none'}`}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input 
                      ...
                      disabled={!passportEnabled}
                      className="w-full bg-white border border-[#e2e8f0] focus:outline-none focus:border-novora p-2.5 rounded-xl font-bold text-slate-800"
                    />
                  </div>
            ...
            <div id="form-step-3" className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="bg-white border border-slate-200/60 rounded-3xl p-6.5 shadow-xs space-y-6">
                ...
                <div className="space-y-6">
                        ...
                        return (
                          <button
                            ...
                            className={`px-5 py-3 text-xs font-semibold rounded-xl border transition-all cursor-pointer select-none min-w-[64px] ${
                              isSelected 
                                ? 'bg-novora/10 border-novora text-novora font-black' 
                  ...
                  {/* Half day off Row */}
                  <div className="space-y-3">
                    <span className="text-[10px] text-slate-400 font-extrabold uppercase tracking-wide bg-amber-50 text-amber-700 px-2.5 py-0.5 rounded">Half day off</span>
                  ...
                  <div className="flex items-center gap-5 pt-3 text-[11px] font-bold text-slate-500">
                    <div className="flex items-center gap-2">
                      <span className="h-3 w-3 rounded bg-novora" />
                ...
                {biometricEnabled && (
                  <div className="space-y-4 pt-1">
                      ...
                      <div key={row.id} className="grid grid-cols-1 md:grid-cols-12 gap-3.5 items-end bg-slate-50/40 p-4.5 rounded-2xl border border-slate-100">
                        ...
                        <div className="md:col-span-5 space-y-1.5">
                          ...
                          <input 
                            ...
                            className="w-full bg-white border border-slate-200 focus:outline-none focus:border-novora p-2.5 rounded-xl font-bold text-slate-800 font-mono"
                          />
                        </div>
                    ...
                    <button 
                      ...
                      className="flex items-center gap-1.5 text-xs font-bold text-novora hover:underline bg-blue-50/50 hover:bg-blue-50 px-3.5 py-2.5 rounded-xl border border-novora/10 transition-colors cursor-pointer w-fit"
                    >
            ...
            <div id="form-step-5" className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="bg-white border border-slate-200/60 rounded-3xl p-6.5 shadow-xs space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2 font-semibold">
                      <div className="flex justify-between">
                        ...
                        <span className="text-novora font-bold">{fullDaysOff.join(', ') || 'None selected'}</span>
                      </div>
                      <div className="flex justify-between">
                ...
                <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4.5 text-xs font-semibold text-slate-600 flex gap-2.5 leading-relaxed">
                  <span className="h-2 w-2 rounded-full bg-novora shrink-0 mt-1.5 animate-pulse" />
        ...
        <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-48">
          ...
          <div className="bg-white border border-slate-200/60 rounded-3xl p-6.5 shadow-xs">
            ...
            <div className="flex justify-between items-baseline mt-1">
              <span className="text-2xl font-black text-slate-850 tracking-tight">{step * 20}%</span>
            ...
            <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden mt-3 border border-slate-50">
              <motion.div 
                className="bg-novora h-full rounded-full" 
                initial={{ width: '20%' }}
            ...
            <ul className="space-y-3.5">
              {systemTips[step - 1].map((tipText, tIdx) => (
                <li key={tIdx} className="flex gap-2.5 items-start text-xs font-semibold text-slate-500 leading-normal">
                  <span className="h-1.5 w-1.5 rounded-full bg-novora shrink-0 mt-1.5" />
                  <span>{tipText}</span>
                </li>
              ))}
            </ul>
        ...
        <div className="flex items-center gap-3">
          ...
          {step > 1 && (
            <button 
              ...
              className="bg-white hover:bg-slate-50 font-bold text-xs text-slate-700 border border-slate-200 px-4.5 py-2.5 rounded-xl transition-all inline-flex items-center gap-2 cursor-pointer shadow-2xs"
            >
