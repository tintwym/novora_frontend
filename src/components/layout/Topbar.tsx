    ...
    >
      <div className="flex items-center gap-2 min-w-0">
        <h1
          ...
          className="text-[18px] md:text-[19px] font-bold text-slate-900 tracking-tight truncate"
          title={activeTabName}
        >
          {title}
        </h1>
      ...
      <div className="flex items-center gap-3 md:gap-4">
        <div id="topbar-search-container" className="relative w-48 sm:w-64 md:w-80 hidden sm:block">
          ...
          <input
            ...
            className="w-full text-sm text-slate-800 bg-slate-50 border border-slate-100 focus:border-[#2f66e0]/30 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#2f66e0]/10 placeholder-slate-400 py-1.5 pl-10 pr-4 rounded-xl transition-all"
          />
        </div>
        ...
        <div id="topbar-notification-bell" className="relative" ref={notifyRef}>
          <button
            ...
            className="p-2.5 rounded-xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50 transition-colors relative cursor-pointer"
          ...
          {notificationsOpen && (
            <div
              ...
              className="absolute right-0 mt-2.5 w-80 bg-white border border-slate-100 rounded-2xl shadow-xl py-2 z-50 animate-soft-fade-up"
            >
              <div className="px-4 py-2.5 border-b border-slate-50 flex items-center justify-between">
                <span className="font-bold text-slate-800 text-sm">Notifications</span>
                <button
                  type="button"
                  className="text-[11px] text-[#2f66e0] font-semibold cursor-pointer hover:underline"
        ...
        <div id="topbar-user-profile" className="relative" ref={profileRef}>
          <button
            ...
            className="flex items-center gap-2 pl-2 pr-2.5 py-1.5 rounded-xl border border-slate-100 hover:border-slate-200 hover:bg-slate-50 transition-colors cursor-pointer"
            title={displayRole}
          ...
          {profileOpen && (
            <div
              ...
              className="absolute right-0 mt-2.5 w-56 bg-white border border-slate-100 rounded-2xl shadow-xl py-1.5 z-50 animate-soft-fade-up"
            >
              <div className="px-4 py-3 border-b border-slate-50">
                ...
                {session?.companyName && (
                  <div className="text-[10px] font-semibold text-[#2f66e0] mt-1 truncate">
                    {session.companyName}
                  </div>
                )}
              </div>
