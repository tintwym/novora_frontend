    ...
    <div className="space-y-5 select-none animate-in fade-in duration-300">
      ...
      <div className="nv-panel p-6 sm:p-8">
        <div className="flex items-start gap-4">
          <div className="h-11 w-11 rounded-xl bg-[#2f66e0]/10 text-[#2f66e0] flex items-center justify-center shrink-0">
        ...
        {capabilities.length > 0 ? (
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
            {capabilities.map((cap) => (
              <div
                key={cap.label}
                className="rounded-xl border border-slate-100 bg-slate-50/70 px-3.5 py-3 flex items-center justify-between gap-2"
              >
