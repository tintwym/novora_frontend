...
import { PayAddButton, PayCard, PayKv, PaySectionTitle, PaySubPills, PayTableScroll, PayToolbarRow } from './PayrollShared'
...
function PaymentDurationView({ onNew }: { onNew: () => void }) {
  return (
    <>
      <PayToolbarRow>
        <p className="pay-muted inline">Define the monthly payroll calculation period</p>
...
function PayrollPreparationView() {
  return (
    <PayCard>
      <PaySectionTitle title="May 2026 pre-disbursement checklist" />
      <PayTableScroll>
        <table className="pay-table">
          <thead>
            <tr>
...
function PayrollRunView({ onRun }: { onRun: () => void }) {
  return (
    <PayCard className="pay-run-card">
      <span className="pay-run-icon" aria-hidden>🧮</span>
      ...
      <button type="button" className="pay-navy-btn full" onClick={onRun}>
