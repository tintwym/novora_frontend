...
export default function PortalShell() {
  const { portal: portalParam, module: moduleParam } = useParams<{
    portal: string
    module?: string
  }>()
  ...
  const requestedTab = tabFromSlug ?? defaultTabFor(session.roles)
  const activeTab: SidebarTab = canAccessTab(session.roles, requestedTab)
    ? requestedTab
    : defaultTabFor(session.roles)

  if (!moduleParam || !tabFromSlug || activeTab !== requestedTab) {
    return <Navigate to={portalPath(portal, activeTab)} replace />
  }
  ...
  const goToTab = (tab: SidebarTab) => {
    ...
    navigate(portalPath(portal, tab))
  }
  ...
  const handleAddEmployee = (newEmp: Employee) => {
    ...
    navigate(portalPath(portal, 'Employees Management'))
  }
  ...
  return (
    <div id="novora-hrms-root" className="flex h-dvh min-h-0 bg-transparent select-none font-sans overflow-hidden">
      <Sidebar
        activeTab={activeTab}
      ...
      <main id="main-portal-contents" className="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        <div id="portal-inner-board" className="flex-1 min-h-0 overflow-y-auto px-5 sm:px-8 py-5 sm:py-6">
                ...
                <div id="employees-top-controls" className="flex items-center gap-3 self-end md:self-auto relative shrink-0 flex-nowrap">
                  ...
                  <div id="export-actions-dropdown" className="relative shrink-0">
                    <button
                      ...
                      className="h-9 inline-flex items-center gap-2 px-4 text-xs font-bold text-slate-700 bg-white border border-slate-200 hover:border-[#2f66e0]/35 hover:bg-[#2f66e0]/5 rounded-xl transition-all cursor-pointer whitespace-nowrap shrink-0"
                    >
                    ...
                    {exportDropdownOpen && (
                      <div
                        ...
                        className="absolute right-0 mt-2 w-44 bg-white border border-slate-200 rounded-xl shadow-lg py-1.5 z-40"
                      >
